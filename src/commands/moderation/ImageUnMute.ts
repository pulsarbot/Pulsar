...
export default class ImageUnMute extends Command {
	//Define the fields for the command
	private static commandFields = new CommandField(
		"iunmute", //NAME
		"image unmute a muted user from a guild", //DESCRIPTION
		"iunmute [@member/userID]", //USAGE - [] = MANDATORY () = OPTIONAL
		["iunmute [@severepain#0001]"], //EXAMPLES
		...
		0, //SPAM TIMEOUT
		["ium", "unimute", "imageperms"] //ALIASES
	);
	...
	constructor(cmdConsole:Console){
		...
		super(ImageUnMute.commandFields, cmdConsole);
	}
	...
	public async run(bot:Pulsar, message:Discord.Message, args:string[], calledName:string):Promise<any> {
		...
		// No arguments
		if (!args[0]) return m.edit(`:information_source: The correct usage is: ${bot.config.prefix}${ImageUnMute.commandFields.usage}`);
		...
		await memberToMute
		// Member Not Found
		if (!memberToMute) return m.edit(`:warning: Couldn't find the member to un-image mute.`);

		let muteRole = message.guild.roles.cache.find(r => r.name === "Image Muted");
		if(!muteRole) return m.edit(`:warning: The image muted role doesn't exist on this server!`);

		let psGuild = await bot.pulsarGuilds.get(message.guild.id);

		let guildConfig: any = psGuild.config;
		let isUserMuted = await psGuild.isUserMuted(`image`, memberToMute.id);

		if(!memberToMute.roles.cache.has(muteRole.id) && isUserMuted){
			let unmuteSuccessful = await psGuild.deleteMutedUserEntry(`image`, memberToMute.id)
			if(!unmuteSuccessful){
			...
			return m.edit(`:no_entry: That person is not image muted!`);
		}

		let reportEmbed = new Discord.MessageEmbed()
			.setAuthor("Image Unmute", message.author.displayAvatarURL())
			...
			if(!isUserMuted && memberToMute.roles.cache.has(muteRole.id)){
				...
				await memberToMute.user.send(`:frame_photo: You have regained your image posting perms on ${message.guild.name}!`)
				return m.edit(`:white_check_mark: User <@${memberToMute.id}> (${memberToMute.user.tag}) has regained their image posting perms!`)
			}
			else if (isUserMuted){
				try {
					let unmuteSuccessful = await psGuild.deleteMutedUserEntry(`image`, memberToMute.id)
