...
export default class VoiceMute extends Command {
	//Define the fields for the command
	private static commandFields = new CommandField(
		"vmute", //NAME
		"Voice mute a user from speaking in voice chat in the guild for a certain amount of time", //DESCRIPTION
		"vmute [@member/userID] (time) (reason)", //USAGE - [] = MANDATORY () = OPTIONAL
		["vmute [@severepain#0001] (60m) (spam)", "vmute [@severepain#0001] (spam)"], //EXAMPLES
		...
		0, //SPAM TIMEOUT
		["vm", "vsilence", "vdeafen", "voicemute"] //ALIASES
	);

	constructor(cmdConsole:Console){
		//Call the superclass with the command fields
		super(VoiceMute.commandFields, cmdConsole);
	}
	...
	public async run(bot:Pulsar, message:Discord.Message, args:string[], calledName:string):Promise<any> {
		...
		// No arguments
		if (!args[0]) return m.edit(`:information_source: The correct usage is: ${bot.config.prefix}${VoiceMute.commandFields.usage}`);

		// Find Member
		var memberToMute: Discord.GuildMember;
		...
		await memberToMute
		// Member Not Found
		if (!memberToMute) return m.edit(`:warning: Couldn't find the member to voice mute.`);

		if(memberToMute.hasPermission(`MANAGE_MESSAGES`)) return m.edit(`:no_entry: You cannot voice mute that member!`);

		let role = message.guild.roles.cache.find(r => r.name === "Voice Muted");
    	...
    	if(memberToMute.roles.cache.has(role.id)) return m.edit(":warning: This person is already voice muted.");

    	// Give member the muted role
   		await memberToMute.roles.add(role).catch(e => {return m.edit(":warning: This member cannot be voice unmuted.");});;
    	await memberToMute.voice.setMute(true, 'User Voice Muted').catch(e => {});
    	...
    	if ((/^\d/.test(muteArgs[0])) && ((muteArgs[0].endsWith('s')) || (muteArgs[0].endsWith('m')) || (muteArgs[0].endsWith('h')) || (muteArgs[0].endsWith('d')) || (/\d+$/.test(muteArgs[0])))){
			...
			let success = await psGuild.createMutedUserEntry(message.author.id, `voice`, memberToMute.id, finalMuteTime, muteReason);
	 ...
	 if(!timeToMute){
		...
		psGuild.voice_muted_users[memberToMute.id] =  {
			"reason": rr,
		...
		fs.writeFileSync(`./data/guilds/${message.guild.id}/voice-muted-users.json`, JSON.stringify(psGuild.voice_muted_users, null, 4));
	}

    // Try to notify the member that they have been muted
    if(muteReason && timeToMute) memberToMute.user.send(`:mute: You have been voice muted on **${message.guild.name}** for ${String(timeToMute/60000)} minutes for the following reason: ${muteReason}. Your case ID is ${psGuild.voice_muted_users[memberToMute.id].caseID}`).catch(e => {})
    else if (muteReason) memberToMute.user.send(`:mute: You have been voice muted on **${message.guild.name}** for the following reason: ${muteReason}. Your case ID is ${psGuild.voice_muted_users[memberToMute.id].caseID}`).catch(e => {})
    else if (timeToMute) memberToMute.user.send(`:mute: You have been voice muted on **${message.guild.name}** for ${String(timeToMute/60000)} minutes. Your case ID is ${psGuild.voice_muted_users[memberToMute.id].caseID}`).catch(e => {})
    else memberToMute.user.send(`:mute: You have been voice muted on **${message.guild.name}**. Your case ID is ${psGuild.voice_muted_users[memberToMute.id].caseID}`).catch(e => {});
	


    timeToMute ? m.edit(`:white_check_mark: <@${memberToMute.id}> (${memberToMute.user.tag}) has been voice muted for ${String(timeToMute/60000)} minutes. Case ID: ${psGuild.voice_muted_users[memberToMute.id].caseID}`) : m.edit(`:white_check_mark: <@${memberToMute.id}> (${memberToMute.user.tag}) has been voice muted. Case ID: ${psGuild.voice_muted_users[memberToMute.id].caseID}`);
    ...
    if(guildConfig.actionChannel) {
        ...
        .setAuthor("Voice Mute", message.author.displayAvatarURL())
        ...
        .setTimestamp()
        .setFooter(`ID: ${psGuild.voice_muted_users[memberToMute.id].caseID}`);
