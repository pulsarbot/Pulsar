...
export default class Cursed extends Command {
	//Define the fields for the command
	private static commandFields = new CommandField(
		"cursed", //NAME
		"Sends a random image of mysterious origin", //DESCRIPTION
		"cursed [none]", //USAGE - [] = MANDATORY () = OPTIONAL
		["cursed [none]"], //EXAMPLES
	...
	constructor(cmdConsole:Console){
		...
		super(Cursed.commandFields, cmdConsole);
	}
		...
		randomPuppy(subreddit).then(async url => {
				let sentMessage = await message.channel.send({
					files: [{
						...
						name: ('cursed-image-' + (await MD5.md5(url)) + '.png')
					}]
				}) as Discord.Message;
